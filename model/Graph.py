...
class NGCF(nn.Module):
    ...
    def _create_lightgcn_embed(self):
        ...
        u_g_embeddings, i_g_embeddings = torch.split(light_out, [self.n_users, self.n_items])
        return u_g_embeddings, i_g_embeddings
    ...
    def forward(self, user_id, pos_id, neg_id):
        ...
        return user_emb_ego, pos_emb_ego, neg_emb_ego, user_emb, pos_emb, neg_emb, pos_i_com
    ...
    def out_forward(self, user, pos, neg, user_embeddings, item_embeddings, drop_flag=True):
        ...
        user_emb_ego = user_embeddings[user_id]
        pos_emb_ego = item_embeddings[pos_id]
        neg_emb_ego = item_embeddings[neg_id]

        batch_label = self.train_label[user_id]
        pos_i_com = torch.matmul(batch_label, item_embeddings)
        num = batch_label.sum(1).unsqueeze(1)
        pos_i_com = pos_i_com / num
